...
def aplicar_threshold(img_gray):
    ...
    _, thresh = cv2.threshold(img_gray, valor_maximo/5*4.9, valor_maximo, cv2.THRESH_BINARY_INV)
    return thresh
...
# Carregando a imagem
img = carregar_imagem('images/GIRAFA.jpeg')
...
edges_gray = detectar_bordas(img_gray, threshold1=img_gray.max()/5, threshold2=img_gray.max()/2)
edges_blur = detectar_bordas(img_blur, threshold1=20, threshold2=190)
